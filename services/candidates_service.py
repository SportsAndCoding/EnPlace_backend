...
class CandidatesService:
    ...
    async def update_candidate(
        ...
        try:
            ...
            if payload.get('status') == 'hired' and 'hired_at' not in payload:
                payload['hired_at'] = datetime.utcnow().isoformat()
            if payload.get('status') in ['hired', 'rejected'] and 'decision_at' not in payload:
                payload['decision_at'] = datetime.utcnow().isoformat()
