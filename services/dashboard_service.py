...
from services.industry_content import get_cautionary_tales, get_category_summaries
...
def _generate_network_report(organization_id: int) -> dict:
    ...
    # Look up the org's industry to select the right content pack
    org_result = supabase.table("organizations").select("industry").eq("id", organization_id).single().execute()
    industry = (org_result.data or {}).get("industry", "restaurant")
    
    today = get_today_for_restaurant(organization_id)
    week_start = today - timedelta(days=today.weekday())
    week_end = week_start + timedelta(days=6)
    
    # Cautionary tales selected by industry, rotated weekly per org
    cautionary_tales = get_cautionary_tales(industry)
    ...
    return {
        ...
        "report_content": {
            ...
            "category_summaries": get_category_summaries(industry),
