...
def get_restaurant_timezone(organization_id: int) -> str:
    """Get timezone string for a restaurant (cached)."""
    if organization_id in _timezone_cache:
        return _timezone_cache[organization_id]
    ...
    _timezone_cache[organization_id] = tz_name
    return tz_name


def get_today(organization_id: int) -> date:
    """Get today's date in the restaurant's timezone."""
    tz_name = get_restaurant_timezone(organization_id)
    tz = pytz.timezone(tz_name)
    return datetime.now(tz).date()


def get_now(organization_id: int) -> datetime:
...
def get_date_range(organization_id: int, days_back: int = 7) -> tuple[date, date]:
    """Get (start_date, end_date) for a range ending today in restaurant timezone."""
    today = get_today(organization_id)
