...
def _get_group_id_for_staff(supabase, staff_id: str) -> int:
    ...
    a staff metadata field. We use organization_id=NULL + notes field
...
async def get_group_summary(
    ...
    # Verify group exists
    group_result = supabase.table("organization_groups").select("id, name").eq(
        "id", group_id
    ).execute()
    ...
    restaurants_result = supabase.table("organizations").select(
    ...
    snapshots_result = supabase.table("restaurant_daily_snapshot").select("*").in_(
        "organization_id", restaurant_ids
    ).eq("snapshot_date", today).execute()

    # Build location list with snapshot data attached
    snapshot_map = {s["organization_id"]: s for s in (snapshots_result.data or [])}
...
@router.get("/group/{group_id}/location/{organization_id}/trend")
async def get_location_trend(
    ...
    r = supabase.table("organizations").select("id, name, location_label, group_id").eq(
        "id", organization_id
    ).execute()
...
async def get_group_alerts(
    ...
    snapshots_result = supabase.table("restaurant_daily_snapshot").select(
        "organization_id, sma_score, flight_risk_critical, flight_risk_high, "
        "active_escalations, checkin_completion_rate, avg_mood_7d, mood_trend"
    ).in_("organization_id", restaurant_ids).eq("snapshot_date", today).execute()

    alerts = []
    for snap in (snapshots_result.data or []):
        rid = snap["organization_id"]
